...
async function getLatestAlphaVersion() {
  if (process.env.CLEW_TEST_MIHOMO_ALPHA_VERSION) {
    META_ALPHA_VERSION = process.env.CLEW_TEST_MIHOMO_ALPHA_VERSION
    return
  }
  if (!FORCE) {
...
async function getLatestReleaseVersion() {
  if (process.env.CLEW_TEST_MIHOMO_STABLE_VERSION) {
    META_VERSION = process.env.CLEW_TEST_MIHOMO_STABLE_VERSION
    return
  }
  if (!FORCE) {
...
async function runTask() {
  ...
  if (!task) return
  if (process.env.CLEW_INTERNAL_TEST_BUILD === '1' && task.name === 'service')
    return runTask()
