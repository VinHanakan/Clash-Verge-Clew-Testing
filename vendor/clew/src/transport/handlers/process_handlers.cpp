...
void handle_process_tree(const httplib::Request&, httplib::Response& res, const api_context& ctx) {
    write_json(res, ctx.processes.list_tree());
}
...
void register_process_handlers(route_registry& r) {
    ...
    r.add({get,     "/api/processes",                       &handle_process_tree});
