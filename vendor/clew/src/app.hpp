...
struct cli_options {
    bool        gui_mode        = false;
    bool        headless        = false;  // --headless: no WebView/tray/autostart UI
    bool        help            = false;
    ...
    std::string api_token;                // --api-token: bearer token for headless writes
    std::string token_file;               // --token-file: protected credential handoff
...
private:
    ...
    [[nodiscard]] nlohmann::json                   helper_status() const;
