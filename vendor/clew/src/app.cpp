...
app::app(const cli_options& opts, HINSTANCE hinstance)
    ...
    , api_server_(API_PORT, ctx_, opts_.static_dir, opts_.headless, opts_.api_token,
                  [this]() { return helper_status(); },
                  [this]() { request_stop(); })
{
    config_svc_.set_headless(opts_.headless);
    config_.load();
    ...
    sync_groups();
    if (!opts_.headless) dns_mgr_.recover_crash_state();
    if (opts_.headless && config_.get_v2().dns.enabled) {
        throw startup_error{"headless mode rejects dns.enabled=true"};
    }
...
int app::run() {
    ...
    wait_for_tree_init();
    if (!start_traffic_layers()) {
        PC_LOG_ERROR("TCP interception layers failed to start");
        return 1;
    }
    ready_ = true;
...
nlohmann::json app::helper_status() const {
    return nlohmann::json{
        {"protocol_version", 1},
        {"headless", opts_.headless},
        {"helper_ready", ready_.load()},
        {"interception", ready_.load() ? "started" : "starting"},
        {"proxy_ready", false},
        {"proxy_ready_owner", "verge"},
        {"rules_revision", config_svc_.rules_revision()},
    };
}
...
bool app::start_traffic_layers() {
    if (!tree_mgr_.is_initialized()) return false;
    ...
    const bool socket_ok = wd_socket_->open();
    if (socket_ok) {
        wd_socket_->start();
    ...
    const bool network_ok = wd_network_->open();
    if (network_ok) {
...
int app::run_main_loop() {
    if (gui_) {
        ...
        while (!exit_requested_.load() && !shut_down_.load())
...
void app::shutdown() noexcept {
    ...
    if (!opts_.headless) dns_mgr_.stop();
