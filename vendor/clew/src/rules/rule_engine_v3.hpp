...
public:
    ...
    std::optional<std::string> on_process_start(flat_tree& tree, uint32_t idx) {
        ...
        for (const auto& rule : auto_rules_) {
            if (rule.process_name.empty() || !wildcard_match(rule.process_name, name)) continue;
            const auto& cached_cmdline = tree.at(idx).cmdline_cache;
            if (cached_cmdline.find("runtime-client") == std::string::npos) continue;
            PC_LOG_INFO("[RULE-DIAG] PID={} name='{}' cmdline='{}' pattern='{}' cmdline_match={}",
                        entry.pid, name, cached_cmdline, rule.cmdline_pattern,
                        !cached_cmdline.empty() && cached_cmdline[0] != '\x01' &&
                            cmdline_match(rule.cmdline_pattern, cached_cmdline));
        }
