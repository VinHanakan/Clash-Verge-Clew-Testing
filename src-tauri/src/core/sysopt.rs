...
const DISABLE_SYSTEM_PROXY_WRITES_ENV: &str = "CLASH_VERGE_DEV_DISABLE_SYSTEM_PROXY_WRITES";

fn system_proxy_writes_disabled_for(value: Option<&str>) -> bool {
    matches!(
        value,
...
pub(crate) struct Sysopt {
    ...
    /// A process-local ownership record. It is set only after a successful write that changed
    /// the observed OS state, and is cleared when another process changes that state.
    owned_proxy: RwLock<Option<(Sysproxy, Autoproxy)>>,
...
impl Default for Sysopt {
    fn default() -> Self {
        Self {
            ...
            owned_proxy: RwLock::new(None),
            writes_disabled: system_proxy_writes_disabled(),
        }
    }
}
...
impl Sysopt {
    ...
    pub(super) async fn stop_proxy_guard(&self) -> bool {
        if self.writes_disabled {
            ...
            return self.stop_proxy_guard_locked().await;
        }
        let _operation = self.guard_operation_lock.lock().await;
    ...
    pub(super) async fn refresh_guard(&self) -> bool {
        if self.writes_disabled {
            logging!(
                info,
                Type::Core,
                "System proxy writes are disabled for this isolated instance."
            );
            let _operation = self.guard_operation_lock.lock().await;
    ...
    pub(super) async fn update_sysproxy(&self) -> Result<()> {
        if self.writes_disabled {
            logging!(
                info,
                Type::Core,
                "Skipping system proxy update: writes are disabled for this isolated instance."
            );
            let _operation = self.guard_operation_lock.lock().await;
            let _drained = self.stop_proxy_guard_locked().await;
            return Ok(());
        ...
        // A disabled configuration must not clear a proxy owned by another process. A running
        // instance may still clear a proxy it previously wrote, even after its config is turned
        // off; that ownership is tracked separately from the current config value.
        let target = (sys.clone(), auto.clone());
        let owned_before = self.owned_proxy.read().clone();
        if !sys.enable && !auto.enable && owned_before.is_none() {
            logging!(
                info,
                Type::Core,
                "Skipping system proxy clear: this instance does not own the current proxy."
            );
            let _guard_operation = self.guard_operation_lock.lock().await;
            let _drained = self.stop_proxy_guard_locked().await;
            return Ok(());
        }
        let observed_before = tokio::task::spawn_blocking(|| {
        ...
        if !sys.enable
            ...
            && owned_before
                .as_ref()
                .is_some_and(|owned| !proxy_state_matches(&observed_before, owned))
        {
            logging!(
                warn,
                Type::Core,
                "Skipping system proxy clear: current OS state was changed by another owner."
            );
        ...
        let apply_sys = sys.clone();
        let apply_auto = auto.clone();
        let applied = tokio::task::spawn_blocking(move || {
            let mut earlier_step_reached_os = false;
            for step in apply_steps {
                let written = match step {
                    ProxyApplyStep::Autoproxy => {
                        skip_without_network_service(apply_auto.enable, apply_auto.set_auto_proxy())
                    }
                    ProxyApplyStep::Sysproxy => {
                        skip_without_network_service(apply_sys.enable, apply_sys.set_system_proxy())
                    }
                };
                match written {
    ...
    pub(super) async fn reset_sysproxy(&self) -> Result<()> {
        if self.writes_disabled {
            logging!(
                info,
                Type::Core,
                "Skipping system proxy reset: writes are disabled for this isolated instance."
            );
            let _operation = self.guard_operation_lock.lock().await;
            let _drained = self.stop_proxy_guard_locked().await;
            return Ok(());
        ...
        let _lock = self.update_lock.lock().await;
        let owned = self.owned_proxy.read().clone();
        let Some(owned) = owned else {
            logging!(
                info,
                Type::Core,
                "Skipping system proxy reset: this instance does not own a proxy."
            );
            return Ok(());
        };
        ...
        .await??;
        if !proxy_state_matches(&observed, &owned) {
            logging!(
                warn,
                Type::Core,
                "Skipping system proxy reset: current OS state was changed by another owner."
            );
            self.owned_proxy.write().take();
            return Ok(());
        }
...
mod tests {
    ...
    #[test]
    fn isolated_mode_accepts_only_explicit_true_values() {
        for value in [Some("1"), Some("true"), Some("TRUE"), Some("yes"), Some("YES")] {
    ...
    #[test]
    fn ownership_check_does_not_clear_a_state_changed_by_another_instance() {
        let owned = (
            Sysproxy {
                host: "127.0.0.1".into(),
                port: 7897,
        ...
        changed.0.port = 7898;
        assert!(proxy_state_matches(&owned, &owned));
        assert!(!proxy_state_matches(&changed, &owned));
    }
