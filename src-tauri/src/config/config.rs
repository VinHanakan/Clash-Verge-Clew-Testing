...
impl Config {
    ...
    pub(crate) async fn generate_with_profiles(profiles: &IProfiles) -> Result<()> {
        ...
        // Re-merge the Verge-owned application listener after profile and
        // subscription generation replaces the runtime mapping.
        crate::core::app_proxy::merge_managed_listener(&mut config);
