...
impl WindowManager {
    ...
    pub fn create_window(should_create: bool) -> Pin<Box<dyn Future<Output = bool> + Send>> {
        Box::pin(async move {
            ...
            eprintln!("[window-debug] create_window should_create={}", should_create);
            ...
            match build_new_window().await {
                Ok(_) => {
                    #[cfg(debug_assertions)]
                    eprintln!("[window-debug] create_window returned created");
                ...
                Err(e) => {
                    #[cfg(debug_assertions)]
                    eprintln!("[window-debug] create_window failed error={}", e);
