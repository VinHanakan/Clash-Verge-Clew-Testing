...
fn ensure_service_mutation_allowed() -> CmdResult {
    if cfg!(feature = "internal-test") {
        return Err(super::CommandFailure::coded(
            "SERVICE_DISABLED_FOR_INTERNAL_TEST",
            "global Clash Verge service changes are disabled in the internal test build",
        ));
    }
    Ok(())
}
...
pub async fn install_service() -> CmdResult {
    ensure_service_mutation_allowed()?;
