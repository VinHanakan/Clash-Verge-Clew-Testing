...
pub async fn get_runtime_config() -> CmdResult<Option<Mapping>> {
    let evidence = cfg!(debug_assertions)
        && std::env::var("CLEW_PHASE3_RUNTIME_EVIDENCE").as_deref() == Ok("1");
    if evidence {
        eprintln!("[phase3-runtime-evidence] get_runtime_config entered");
    }
    let config = Config::runtime().await.latest_arc().config.clone();
    if evidence {
        eprintln!("[phase3-runtime-evidence] get_runtime_config lock_read complete");
    }
    if std::env::var("CLEW_PHASE3_RUNTIME_EVIDENCE").as_deref() == Ok("1") {
        let listeners = config
            .as_ref()
            .and_then(|value| value.get("listeners"))
            .map(|value| format!("{value:?}"))
            .unwrap_or_else(|| "<none>".to_owned());
        tracing::info!("[phase3-runtime-evidence] listeners={listeners}");
    }
    if evidence {
        eprintln!("[phase3-runtime-evidence] get_runtime_config returned");
    }
    Ok(config)
}
