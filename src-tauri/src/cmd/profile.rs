...
pub async fn patch_profiles_config(profiles: IProfiles) -> CmdResult<ValidationOutcome> {
    crate::core::app_proxy::ensure_profile_switch_allowed()
        .map_err(|error| coded_error("APP_PROXY_RUNNING", error))?;
...
pub async fn patch_profiles_config_by_profile_index(
    ...
    logging!(
        info,
        Type::Cmd,
        "profile command=patch_profiles_config_by_profile_index phase=entered target={} correlation_id={}",
        profile_index,
        correlation_id
    );
    ...
    let result = patch_profiles_config(profiles).await;
    match &result {
        Ok(outcome) => logging!(
            info,
            Type::Cmd,
            "profile command=patch_profiles_config_by_profile_index phase=returned target={} outcome={:?} correlation_id={}",
            profile_index,
            outcome,
            correlation_id
        ),
        Err(error) => logging!(
            error,
            Type::Cmd,
            "profile command=patch_profiles_config_by_profile_index phase=failed target={} correlation_id={} error={:#}",
            profile_index,
            correlation_id,
            error
        ),
